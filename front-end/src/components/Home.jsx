...
const updateTask = async () => {
  try {
    const res = await axios.patch(
      `http://localhost:5000/tasks/${userId}/${editTask._id}`,
      {
        title: editTitle,
 ...
 try {
      await axios.patch(`http://localhost:5000/tasks/${userId}/${taskId}`, {
        status: !status,
      });
  ...
  const  deleteTask = async (taskId) => {
    ...
    await axios.delete(`http://localhost:5000/tasks/${userId}/${taskId}`);
